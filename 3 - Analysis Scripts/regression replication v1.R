...
library(lubridate); library(scales); library(IndexNumR); library(wid); library(plm)  
...
regression_ready_df <- mega_combined_vars_df_final %>%
  ...
  make.pbalanced()
#-----------------------------------------------------------------------------------------------
form_m1.1 <- wid_gini_coeff ~
  pwt_irr +
  oecd_union_density +
  log(pwt_gdp_per_capita_2017_USD) +
  wid_ratio_capital_share_factor_price_national_income +
  pwt_human_capital_index +
  pwt_pct_gdp_international_trade +
  oecd_public_socspend_wa_pct_gdp +
...
form_m1.4 <- wid_ratio_income_share_90th_50th_pctile ~
  pwt_irr +
  oecd_union_density +
  log(pwt_gdp_per_capita_2017_USD) +
  wid_ratio_capital_share_factor_price_national_income +
...
form_m1.5 <- wid_ratio_income_share_90th_10th_pctile ~
  pwt_irr +
  oecd_union_density +
  log(pwt_gdp_per_capita_2017_USD) +
  wid_ratio_capital_share_factor_price_national_income +
...
form_m1.6 <- wid_pre_tax_income_share_top_10pct ~
  pwt_irr +
  oecd_union_density +
  log(pwt_gdp_per_capita_2017_USD) +
  wid_ratio_capital_share_factor_price_national_income +
...
form_m1.7 <- wid_pre_tax_income_share_top_1pct ~
  pwt_irr +
  oecd_union_density +
  log(pwt_gdp_per_capita_2017_USD) +
  wid_ratio_capital_share_factor_price_national_income +
...
lm_m5 <- lm(form_m1.5, data = regression_ready_df)
lm_m6 <- lm(form_m1.6, data = regression_ready_df)
lm_m7 <- lm(form_m1.7, data = regression_ready_df)
#-----------------------------------------------------------------------------------------------
...
hc_se_lm_m7 <- sqrt(diag(vcovHC(lm_m7, type = "HC1")))
#-----------------------------------------------------------------------------------------------
...
pcse_se_lm_m7 <- sqrt(diag(vcovPC(lm_m7, cluster = ~ iso3 + year)))
#-----------------------------------------------------------------------------------------------
# Creating List of LM Objects
#-----------------------------------------------------------------------------------------------
lm_group <- list(lm_m1, lm_m2, lm_m3, lm_m4, lm_m5, lm_m6, lm_m7)
#-----------------------------------------------------------------------------------------------
hc.se_group <- list(hc_se_lm_m1, hc_se_lm_m2, hc_se_lm_m3, hc_se_lm_m4, hc_se_lm_m5, hc_se_lm_m6, hc_se_lm_m7)
#-----------------------------------------------------------------------------------------------
pcse.se_group <- list(pcse_se_lm_m1, pcse_se_lm_m2, pcse_se_lm_m3, pcse_se_lm_m4, pcse_se_lm_m5, pcse_se_lm_m6, pcse_se_lm_m7)
#-----------------------------------------------------------------------------------------------
stargazer(lm_group,
          se = c(pcse.se_group),
          type = "text",

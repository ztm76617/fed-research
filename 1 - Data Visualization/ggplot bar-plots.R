...
library(WDI); library(lmtest); library(sandwich); library(interactions); library(ggpubr); library(gplots); library(purrr); library(countrycode)
#--------------------------------------------------------------------------------------
...
countries <- c("CHN", "USA", "GBR", "FRA", "JPN", "CAN")
...
pwt_mega_df %>%
  ...
  filter(iso3 %in% c("CHN", "USA", "GBR", "FRA", "JPN", "CAN")) %>%
  filter(year == 2017) %>%
  mutate(countries = countrycode(iso3, origin = 'iso3c',
                                 destination = 'p4.name')) %>%
  select(countries,

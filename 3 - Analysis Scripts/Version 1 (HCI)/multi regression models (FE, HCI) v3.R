...
regression_ready_df <- mega_combined_vars_df_final %>%
  filter(iso3 %in% c(g20_countries)) %>%
  filter(year >= 1980) %>%
  select(iso3, year,
         ...
         interest_rate_short_terme,
         piketty_ratio)
#-----------------------------------------------------------------------------------------------
form_m1.1 <- wid_ratio_income_share_90th_50th_pctile ~
  pwt_irr +
  cpds_inflation_rate +
  oecd_union_density +
  pwt_gdp_pc_growth_2017_USD +
...
form_m1.2 <- wid_ratio_income_share_90th_50th_pctile ~
  pwt_irr +
  cpds_inflation_rate +
  oecd_union_density +
  pwt_gdp_pc_growth_2017_USD +
...
form_m1.3 <- wid_ratio_income_share_90th_50th_pctile ~
  pwt_irr +
  cpds_inflation_rate +
  oecd_union_density +
  pwt_gdp_pc_growth_2017_USD +
